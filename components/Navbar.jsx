...
export default function Navbar({actualPage}) {
    ...
    return (
        <nav className="flex items-center py-6 px-10 w-full">
            <div className="flex items-center flex-shrink-0 text-white mr-6">
                <span className={`font-bold sm:text-lg text-xs tracking-tight ${lexend.className}`}>
                    ANDIGAMES
                </span>
            </div>
            <div className="block text-[10px] lg:text-xs font-semibold">
                <ul className="flex">
                    <li className="mr-6">
                    ...
                    {/* <li className="mr-6">
                        <a className={actualPage === 'home/what-to-play' ? "text-neutral-600 cursor-not-allowed" : "text-white hover:text-blue-600"} onClick={() => router.push('/home')}>¿Qué jugar?</a>
                    </li> */}
                </ul>
            </div>
        </nav>
